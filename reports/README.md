...
--- During the project, we used a total of approximately 6.17 USD in cloud credits across the group. The service that accounted for the largest share of the cost was Compute Engine with 5.57 USD, which was mainly used when running virtual machines for model training and experimentation. This was expected, as GPU-enabled or long-running VM instances quickly become the most expensive component compared to more lightweight services. Smaller costs were also incurred from Cloud Storage, Cloud Run, Artifact Registry, and Networking, but these were relatively minor in comparison. Overall, working in the cloud was both useful and challenging. On the positive side, cloud services made it possible to deploy APIs, store artifacts, and run experiments without relying on local hardware. At the same time, the cloud environment introduced additional complexity, particularly around configuration, permissions, deployment, and cost awareness. Small mistakes, such as leaving services running longer than necessary, could quickly increase costs. ---
